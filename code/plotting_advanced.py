...
if x == "yes" or x == "y":
    ...
    axs[0].set_title("Temperature Readings")
    axs[0].set_ylabel("Temperature (°C)")
    axs[0].axhline(max(temp_values_bme), color='black', linewidth=1, linestyle='--', label=f"Max Temp ({max(temp_values_bme):.1f}°C)")
    axs[0].axhline(min(temp_values_bme), color='black', linewidth=1, linestyle='--',
                   label=f"Max Temp ({min(temp_values_bme):.1f}°C)")
    temp_median_bme = np.median(temp_values_bme)
    ...
    axs[1].set_title("Humidity Readings")
    axs[1].set_ylabel("Humidity (%)")
    axs[1].axhline(max(hum_values_bme), color='black', linewidth=1, linestyle='--', label=f"Max Hum ({max(hum_values_bme):.1f} %)")
    axs[1].axhline(min(hum_values_bme), color='black', linewidth=1, linestyle='--',
                   label=f"Max Hum ({min(hum_values_bme):.1f} %)")
    hum_median_bme = np.median(hum_values_bme)
    ...
    axs[2].set_title("Pressure Readings")
    axs[2].set_ylabel("Pressure (hPa)")
    axs[2].axhline(max(pres_values), color='purple', linewidth=1, linestyle='--', label=f"Max Pressure ({max(pres_values):.1f}hPa)")
    axs[2].axhline(min(pres_values), color='purple', linewidth=1, linestyle='--',
                   label=f"Max Pressure ({min(pres_values):.1f}°C)")
    pres_median = np.median(pres_values)
    axs[2].axhline(pres_median, color='green', linewidth=1, linestyle='--',
                   label=f"Median Pressure ({pres_median:.1f} hPa)")
    axs[2].legend()
