...
|    14   |      Work on the documentation on GitHub, adding new information and fixing what we have already done     |        Documented methodology, success criteria, and explained tools and setup.        |     70 min     |          Dec 5         |  A, B, C  |
...
|    17   |                                   Continue Working on Criterion B and C                                   |    Completed the test plan, noted project limitations, and finalized documentation.    |     50 min     |          Dec 6         |    B, C   |
...
|    19   |                                       Create a Science Poster that                                        |   Developed a science poster covering introduction, methods, results, and conclusion.  |     115 min    |          Dec 7         |     D     |
|    20   |                 Continue working on the Science Poster, fix all occurring glitches and bug                |                Reviewed and refined the poster for clarity and accuracy.               |     25 min     |          Dec 8         |     D     |
|    21   |                                        Film the Demonstration Video                                       |        Created a 6-7 minute video demonstrating product functionality and code.        |     45 min     |          Dec 8         |     D     |
|    22   |                          Update the Criterion D, add final touches to the project                         |       Added a link to the Science Poster, fixed some small bugs in the repository      |     25 min     |          Dec 8         |     D     |
## Test Plan
|       Planned Action      |                                               Procedure                                              |            | Test Number | Success Criteria | Procedure                                                                                                                                                                                                                                                                                                                                 | Expected Outcome                                                                                                                        |
|-------------|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| 1           | 1                | 1. Upload the data to the API server 2. Retrieve the readings from the API server for each sensor 3. Graph each of the sensors using the `pyplot` library                                                                                                                                                                                 | The graphs are continuous and display at least 2880 data points, as the recordings are performed every minute for 48 hours.             |
| 2           | 2                | 1. Connect the BME280 (device has temperature, humidity, and pressure sensor) to the Raspberry Pi 2. Connect the DHT11 (device has temperature and humidity sensor) to the Raspberry Pi 3. Run the code that uploads data from each device to the weather API each minute for 5 minutes 4. Check the JSON file returned for each API call | The JSON confirms that a real value has been uploaded to the server.                                                                    |
| 3           | 3                | 1. Retrieve the sensors' data 2. Run each sensor's data through the mathematical model 3. Graph the data of each sensor using pyplot and the mathematical model on the same graphs                                                                                                                                                        | The graph of the mathematical model should resemble the data collected.                                                                 |
| 4           | 4                | 1. Retrieve the sensor's data 2. Graph each sensor's data 3. Add the graph of the ideal range for each environmental variable to the graph of the corresponding sensors. 4. Generate the composite graph.                                                                                                                                 | The composite graph displays both the ideal range of the environmental variable and the data recorded about the environmental variable. |
| 5           | 5                | 1. Run the code that records data from the sensor and saves the data into a CSV file (the file named amongus.py) 2. Check the CSV file's which name is outputted in the console and the JSON printed in the console                                                                                                                       | The CSV should have data in it and the data in it agrees with the JSON returned by the server.                                          |
| 6           | 6                | 1. Generate the 12-hour prediction for the data using the function we created 2. Generate the graph of the 12-hour prediction and the first 12 hours of the data                                                                                                                                                                          | The prediction should resemble the data, but not an exact copy of it, as it is a prediction, not a copy.                                |
| 7           | 7                | 1. Generates graph of the data with the mathematical model with the ideal levels 2. Generate the graph of the 12-hour prediction 3. Deduce a conclusion about the dormitory environment from the results 4. Make the recommendation about the dormitory environment                                                                       | The client understands the data, the conclusion, and the recommednations                                                                |
